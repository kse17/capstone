...
public class UpdateHomeView {
    ...
    public UpdateHomeView(Activity activity) {
    ...
    public void updateView(int state) {
        if(null != mImgState)
        {
            mImgState.setImageDrawable(mActivity.getDrawable(mainStateImageArray[state]));
        }

        if(null != mImgLock)
        {
            mImgLock.setImageDrawable(mActivity.getDrawable(mainLockImageArray[state]));
        }

        if(null != mTxtState)
        {
            mTxtState.setText(mainStateTextArray[state]);
        }

        if(null != mTxtLock)
        {
            mTxtLock.setText(mainLockTextArray[state]);
        }
    }
}
