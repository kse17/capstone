    ...
    if touch ==1: #touch
        if get_fingerprint():
            ...
            if finger.confidence>70:
                is_open=1-is_open  #open <->-close
                GPIO.output(3, is_open)
                if is_open==1:
                    print("Opened")
                    db.child('groups').update({"operation":0,"result":"door opened"})
                
                    time.sleep(5)
                else:
                    print('closed')
                    db.child('groups').update({"operation":0,"result":"door closed"})
        ...
        result=db.child('groups').get()
        msg=result.val()
        #print(msg['result'])
        #print(msg['operation'])
        #smart phone control
        c = msg['operation']
        new_finger=msg['newfinger']
        if c == 1: #enroll : 1 
            enroll_finger(new_finger)
            db.child('groups').update({"operation":0,"result":str(new_finger)+" : new fingers added"})
        elif c== 2: #open
            is_open=1-is_open  #open <->-close
            GPIO.output(3, is_open)
            if is_open==1:
